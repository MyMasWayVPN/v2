...
MYIP=$(curl -sS ipv4.icanhazip.com)
NameUser=$(curl -sS https://raw.githubusercontent.com/MyMasWayVPN/MyMasWayVPN.github.io/main/wkwkwkwk | grep $MYIP | awk '{print $2}')
...
Save_And_Exit () {
    DATE=$(date +'%d %B %Y')
    cd /root/user-backup
    git config --global user.email "khoirul110604@gmail.com" &> /dev/null
    ...
    git push -f https://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@github.com/MyMasWayVPN/Backup.git &> /dev/null
}
